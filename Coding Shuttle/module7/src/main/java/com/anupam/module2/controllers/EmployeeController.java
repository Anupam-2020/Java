...
import com.anupam.module2.services.impl.EmployeeServiceImpl;
...
public class EmployeeController {
    ...
    public ResponseEntity<List<EmployeeDTO>> getAllEmployeesByEmail(@RequestParam(required = false, name = "email") String email) {
        return ResponseEntity.ok(employeeServiceImpl.getEmployeesByEmail(email));
    }

    @PostMapping
    public ResponseEntity<EmployeeDTO> createEmployee(@Valid @RequestBody EmployeeDTO employee) {
        return new ResponseEntity<>(employeeServiceImpl.createNewEmployee(employee), HttpStatus.CREATED);
    }
    ...
    public ResponseEntity<EmployeeDTO> updatePartialEmployee(@RequestBody @Valid Map<String, Object> updates, @PathVariable Long employeeId) {
        EmployeeDTO employeeDTO = employeeServiceImpl.updatePartialEmployee(employeeId, updates);
